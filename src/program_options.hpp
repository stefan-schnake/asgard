...
static pde_map_t const pde_mapping = {
    ...
    {"vlasov_source", PDE_descriptor("Vlasov lb full f. df/dt == -v*grad_x f + div_v( "
                              "(v-u)f + theta*grad_v f + source)",
                              PDE_opts::vlasov_lb_full_f_source)}};
